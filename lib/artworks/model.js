...
let { MAX_PAGE_SIZE, PAGE_SIZE, ARTSY_URL, THROTTLE_TIME } = process.env
MAX_PAGE_SIZE = Number(MAX_PAGE_SIZE)
PAGE_SIZE = Number(PAGE_SIZE)
THROTTLE_TIME = Number(THROTTLE_TIME)
...
let fetching = {}
let throttledFetchAndUpdate = (id, query) => {
  if (fetching[id]) return
  fetching[id] = true
  fetchAndUpdate(id, query, () => {
    setTimeout((() => fetching[id] = null), THROTTLE_TIME)
  })
}
